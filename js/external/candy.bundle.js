...
Candy.Core.Action = (function(self, Strophe, $) {
	...
	self.Jabber = {
		...
		Room: {
			 ...
			 */
			Join: function(roomJid, password, createRoom) {
			 ...
			 * 
			 */
			CreateInstantRoom : function(roomJid){
				self.Jabber.Room.Disco(roomJid);
				var d = $pres({"from":Candy.Core.getUser().getNick(),"to":roomJid}).c("x",{"xmlns":"http://jabber.org/protocol/muc"});
				Candy.Core.getConnection().send(d.tree());
				Candy.Core.getConnection().muc.createInstantRoom(roomJid);
			},
			
			/**
			 * 
			 */
			SaveConfiguration : function(roomJid, roomConfiguration){
				Candy.Core.getConnection().muc.saveConfiguration(roomJid,roomConfiguration);
			},
			
			/**
			 * 
			 */			
			Configure 	: function(roomJid){
				self.Jabber.Room.Disco(roomJid);
				Candy.Core.getConnection().muc.getConfigure(roomJid);			},
